...
def load_dimension_info(json_dir, dimension, lang):
    ...
    for prompt_dict in full_prompt_list:
        if dimension in prompt_dict['dimension'] and 'video_list' in prompt_dict:
            ...
            elif 'idx' in prompt_dict:
                prompt_dict_ls += [{'prompt': prompt, 'video_list': cur_video_list, 'idx': prompt_dict['idx']}]
            else:
        ...
        # Assign the DINO model path for subject consistency dimension
            ...
            if local:
                ...
                if not os.path.isfile(vit_l_path):
                    ...
                    subprocess.run(wget_command, check=True)
                sa_vit_linear_path = f"{aes_path}/sa_0_4_vit_l_14_linear.pth"
                if not os.path.isfile(sa_vit_linear_path):
                    wget_command = ['wget' ,'https://github.com/LAION-AI/aesthetic-predictor/raw/refs/heads/main/sa_0_4_vit_l_14_linear.pth', '-P', os.path.dirname(sa_vit_linear_path)]
                    subprocess.run(wget_command, check=True)
            else:
                vit_l_path = 'ViT-L/14'
            ...
            if not os.path.exists(submodules_dict[dimension]['pretrain']):
                ...
                subprocess.run(wget_command, check=True)
            bpe_path = f'{CACHE_DIR}/ViCLIP/bpe_simple_vocab_16e6.txt.gz'
            if not os.path.exists(bpe_path):
                wget_command = ['wget', 'https://huggingface.co/OpenGVLab/ViCLIP-L-14-hf/resolve/main/bpe_simple_vocab_16e6.txt.gz', '-P', os.path.dirname(bpe_path)]
                subprocess.run(wget_command, check=True)
